...
module fft_4point_tb(out0, out1, out2, out3, out0_16, out1_16, out2_16, out3_16); // list all the outputs here for the testbench
reg clk;
reg reset;
reg start;
reg [31:0] in0;
...
output [31:0] out0; // you need outputs no matter what! Otherwise, Quartus will be mad...
output [31:0] out1;
output [31:0] out2;
output [31:0] out3;
output [15:0] out0_16;
...
fft_32bit UUT1(.clk(clk), .reset(reset), .start(start), .in0(in0), .in1(in1), .in2(in2), .in3(in3),
	.out0(out0), .out1(out1), .out2(out2), .out3(out3), .done(done)); // fft_4point_32bit
fft_16bit UUT2(.clk(clk), .reset(reset), .start(start), .in0(in0[31:16]), .in1(in1[31:16]), .in2(in2[31:16]), .in3(in3[31:16]), 
	.out0(out0_16), .out1(out1_16), .out2(out2_16), .out3(out3_16), .done(done_16)); // fft_4point_16bit // change to fft_16bit
endmodule
// to do: add 16 bit version
// to do first: finish 32 bit version
